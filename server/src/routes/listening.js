...
router.get('/quiz', requireUser, (req, res) => {
  ...
  const allVocab = db.prepare(vocabQuery).all(...params);
...
router.post('/submit', requireUser, (req, res) => {
  ...
  for (const answer of answers) {
    ...
    if (qq) {
      recordAttempt(req.userId, qq.id, answer.lesson_id, answer.user_answer, answer.correct_answer, isCorrect, 'listening');
    }

 ...
 * @returns {Promise<{ total_xp: number, level: number, leveled_up: boolean }>}
 */
export async function awardXP(userId, xpAmount) {
  const progress = await db.prepare('SELECT * FROM user_progress WHERE user_id = ?').get(userId);
 ...
 */
export async function recordAttempt(userId, questionId, lessonId, userAnswer, correctAnswer, isCorrect, mode = 'standard', timeTakenMs = null) {
  await db.prepare(`
